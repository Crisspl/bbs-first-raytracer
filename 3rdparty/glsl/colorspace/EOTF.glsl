...
NBL_GLSL_API vec3 nbl_glsl_eotf_identity(in vec3 nonlinear)
{
    return nonlinear;
}

NBL_GLSL_API vec3 nbl_glsl_eotf_impl_shared_2_4(in vec3 nonlinear, in float vertex)
{
...
NBL_GLSL_API vec3 nbl_glsl_eotf_sRGB(in vec3 nonlinear)
{
...
NBL_GLSL_API vec3 nbl_glsl_eotf_Display_P3(in vec3 nonlinear)
{
    return nbl_glsl_eotf_impl_shared_2_4(nonlinear, 0.039000312);
}


NBL_GLSL_API vec3 nbl_glsl_eotf_DCI_P3_XYZ(in vec3 nonlinear)
{
    return pow(nonlinear * 52.37, vec3(2.6));
}

NBL_GLSL_API vec3 nbl_glsl_eotf_SMPTE_170M(in vec3 nonlinear)
{
...
NBL_GLSL_API vec3 nbl_glsl_eotf_SMPTE_ST2084(in vec3 nonlinear)
{
...
NBL_GLSL_API vec3 nbl_glsl_eotf_HDR10_HLG(in vec3 nonlinear)
{
...
NBL_GLSL_API vec3 nbl_glsl_eotf_AdobeRGB(in vec3 nonlinear)
{
    return pow(nonlinear, vec3(2.19921875));
}

NBL_GLSL_API vec3 nbl_glsl_eotf_Gamma_2_2(in vec3 nonlinear)
{
    return pow(nonlinear, vec3(2.2));
}


NBL_GLSL_API vec3 nbl_glsl_eotf_ACEScc(in vec3 nonlinear)
{
...
NBL_GLSL_API vec3 nbl_glsl_eotf_ACEScct(in vec3 nonlinear)
{
