...
NBL_GLSL_API vec3 nbl_glsl_oetf_identity(in vec3 linear)
{
    return linear;
}

NBL_GLSL_API vec3 nbl_glsl_oetf_impl_shared_2_4(in vec3 linear, in float vertex)
{
...
NBL_GLSL_API vec3 nbl_glsl_oetf_sRGB(in vec3 linear)
{
...
NBL_GLSL_API vec3 nbl_glsl_oetf_Display_P3(in vec3 linear)
{
    return nbl_glsl_oetf_impl_shared_2_4(linear, 0.0030186);
}

NBL_GLSL_API vec3 nbl_glsl_oetf_DCI_P3_XYZ(in vec3 linear)
{
    return pow(linear / 52.37, vec3(1.0 / 2.6));
}

NBL_GLSL_API vec3 nbl_glsl_oetf_SMPTE_170M(in vec3 linear)
{
...
NBL_GLSL_API vec3 nbl_glsl_oetf_SMPTE_ST2084(in vec3 linear)
{
...
NBL_GLSL_API vec3 nbl_glsl_oetf_HDR10_HLG(in vec3 linear)
{
...
NBL_GLSL_API vec3 nbl_glsl_oetf_AdobeRGB(in vec3 linear)
{
    return pow(linear, vec3(1.0 / 2.19921875));
}

NBL_GLSL_API vec3 nbl_glsl_oetf_Gamma_2_2(in vec3 linear)
{
    return pow(linear, vec3(1.0 / 2.2));
}

NBL_GLSL_API vec3 nbl_glsl_oetf_ACEScc(in vec3 linear)
{
...
NBL_GLSL_API vec3 nbl_glsl_oetf_ACEScct(in vec3 linear)
{
