...
NBL_GLSL_API vec3 nbl_glsl_fresnel_schlick(in vec3 F0, in float VdotH)
{
    float x = 1.0f - VdotH;
    return F0 + (vec3(1.0f) - F0) * x*x*x*x*x;
}

// TODO: provide a `nbl_glsl_fresnel_conductor_impl` that take `Eta` and `EtaLen2` directly
// conductors, only works for `CosTheta>=0`
NBL_GLSL_API vec3 nbl_glsl_fresnel_conductor(vec3 Eta, vec3 Etak, float CosTheta)
{  
   const float CosTheta2 = CosTheta*CosTheta;
   const float SinTheta2 = 1.0 - CosTheta2;

   const vec3 EtaLen2 = Eta*Eta+Etak*Etak;
   const vec3 etaCosTwice = Eta*CosTheta*2.0f;
...
// dielectrics
NBL_GLSL_API float nbl_glsl_fresnel_dielectric_common(in float orientedEta2, in float AbsCosTheta)
{
    const float SinTheta2 = 1.0-AbsCosTheta*AbsCosTheta;

    // the max() clamping can handle TIR when orientedEta2<1.0
    const float t0 = sqrt(max(orientedEta2-SinTheta2,0.0f));
...
NBL_GLSL_API vec3 nbl_glsl_fresnel_dielectric_common(in vec3 orientedEta2, in float AbsCosTheta)
{
   const float SinTheta2 = 1.0-AbsCosTheta*AbsCosTheta;
...
NBL_GLSL_API vec3 nbl_glsl_fresnel_dielectric_frontface_only(in vec3 Eta, in float CosTheta)
{
    return nbl_glsl_fresnel_dielectric_common(Eta*Eta,CosTheta);
}
NBL_GLSL_API vec3 nbl_glsl_fresnel_dielectric(vec3 Eta, in float CosTheta)
{
...
NBL_GLSL_API vec3 nbl_glsl_thindielectric_infinite_scatter(in vec3 singleInterfaceReflectance)
{
    const vec3 doubleInterfaceReflectance = singleInterfaceReflectance*singleInterfaceReflectance;
    return mix((singleInterfaceReflectance-doubleInterfaceReflectance)/(vec3(1.0f)-doubleInterfaceReflectance)*2.0f,vec3(1.0f),greaterThan(doubleInterfaceReflectance,vec3(0.9999f)));
}
NBL_GLSL_API float nbl_glsl_thindielectric_infinite_scatter(in float singleInterfaceReflectance)
{
