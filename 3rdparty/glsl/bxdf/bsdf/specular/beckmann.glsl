...
#include <3rdparty/glsl/bxdf/bsdf/specular/common.glsl>

NBL_GLSL_API nbl_glsl_LightSample nbl_glsl_beckmann_dielectric_cos_generate_wo_clamps(in vec3 localV, in bool backside, in vec3 upperHemisphereLocalV, in mat3 m, vec3& u, in float ax, in float ay, in float rcpOrientedEta, in float orientedEta2, in float rcpOrientedEta2, out nbl_glsl_AnisotropicMicrofacetCache& _cache)
{
    // thanks to this manouvre the H will always be in the upper hemisphere (NdotH>0.0)
    const vec3 H = nbl_glsl_beckmann_cos_generate_wo_clamps(upperHemisphereLocalV,vec2(u),ax,ay);
...
NBL_GLSL_API nbl_glsl_LightSample nbl_glsl_beckmann_dielectric_cos_generate(in nbl_glsl_AnisotropicViewSurfaceInteraction interaction, vec3& u, in float ax, in float ay, in float eta, out nbl_glsl_AnisotropicMicrofacetCache& _cache)
{
...
// isotropic PDF
NBL_GLSL_API float nbl_glsl_beckmann_dielectric_pdf_wo_clamps(in bool transmitted, in float reflectance, in float ndf, in float absNdotV, in float NdotV2, in float VdotH, in float LdotH, in float VdotHLdotH, in float a2, in float orientedEta, out float& onePlusLambda_V)
{
...
// anisotropic PDF
NBL_GLSL_API float nbl_glsl_beckmann_dielectric_pdf_wo_clamps(in bool transmitted, in float reflectance, in float ndf, in float absNdotV, in float TdotV2, in float BdotV2, in float NdotV2, in float VdotH, in float LdotH, in float VdotHLdotH, in float ax2, in float ay2, in float orientedEta, out float& onePlusLambda_V)
{
...
NBL_GLSL_API float nbl_glsl_beckmann_dielectric_cos_remainder_and_pdf_wo_clamps(out float& pdf, in float ndf, in bool transmitted, in float NdotL2, in float absNdotV, in float NdotV2, in float VdotH, in float LdotH, in float VdotHLdotH, in float reflectance, in float orientedEta, in float a2)
{
...
NBL_GLSL_API float nbl_glsl_beckmann_dielectric_cos_remainder_and_pdf(out float& pdf, in nbl_glsl_LightSample _sample, in nbl_glsl_IsotropicViewSurfaceInteraction interaction, in nbl_glsl_IsotropicMicrofacetCache _cache, in float eta, in float a2)
{
...
NBL_GLSL_API float nbl_glsl_beckmann_aniso_dielectric_cos_remainder_and_pdf(out float& pdf, in nbl_glsl_LightSample _sample, in nbl_glsl_AnisotropicViewSurfaceInteraction interaction, in nbl_glsl_AnisotropicMicrofacetCache _cache, in float eta, in float ax, in float ay)
{    
...
NBL_GLSL_API float nbl_glsl_beckmann_smith_height_correlated_dielectric_cos_eval_wo_clamps(
...
NBL_GLSL_API float nbl_glsl_beckmann_smith_height_correlated_dielectric_cos_eval_wo_cache_validation(in nbl_glsl_LightSample _sample, in nbl_glsl_IsotropicViewSurfaceInteraction interaction, in nbl_glsl_IsotropicMicrofacetCache _cache, in float eta, in float a2)
{
    float orientedEta, dummy;
    const bool backside = nbl_glsl_getOrientedEtas(orientedEta, dummy, _cache.VdotH, eta);
    const float orientedEta2 = orientedEta*orientedEta;
...
NBL_GLSL_API float nbl_glsl_beckmann_aniso_smith_height_correlated_dielectric_cos_eval_wo_clamps(
...
// before calling you must ensure that `nbl_glsl_AnisotropicMicrofacetCache` is valid (if a given V vector can "see" the L vector)
NBL_GLSL_API float nbl_glsl_beckmann_aniso_smith_height_correlated_cos_eval_wo_cache_validation(in nbl_glsl_LightSample _sample, in nbl_glsl_AnisotropicViewSurfaceInteraction interaction, in nbl_glsl_AnisotropicMicrofacetCache _cache, in float eta, in float ax, in float ax2, in float ay, in float ay2)
{
    const float TdotH2 = _cache.TdotH*_cache.TdotH;
    const float BdotH2 = _cache.BdotH*_cache.BdotH;
