...
#include <3rdparty/glsl/material_compiler/common_declarations.glsl>
...
#include <3rdparty/glsl/bxdf/brdf/diffuse/fresnel_correction.glsl>
...
#include <3rdparty/glsl/bxdf/bsdf/specular/ggx.glsl>
#include <3rdparty/glsl/bxdf/brdf/specular/blinn_phong.glsl>
#include <3rdparty/glsl/bxdf/bsdf/diffuse/lambert.glsl>
#include <3rdparty/glsl/bxdf/bsdf/specular/dielectric.glsl>
#ifdef TEX_PREFETCH_STREAM
#include <3rdparty/glsl/bump_mapping/utils.glsl>
#endif
...
)
{
	...
	#if GEN_CHOICE_STREAM>=GEN_CHOICE_WITH_AOV_EXTRACTION
	#ifdef ALL_ISOTROPIC_BXDFS
	retval.aovThroughputFactor = nbl_glsl_MC_aov_t_specularThroughputFactor(a2);
	#else
	retval.aovThroughputFactor = nbl_glsl_MC_aov_t_specularThroughputFactor(ax2,ay2);
	#endif
	#endif
		...
		#endif
		{
			if (NdotV>nbl_glsl_FLT_MIN)
			{
				...
				if (nbl_glsl_MC_op_hasSpecular(op))
				{
					...
					#if GEN_CHOICE_STREAM>=GEN_CHOICE_WITH_AOV_EXTRACTION
					if (no_coat_parent)
						out_values.aov.throughputFactor = ctFactors.aovThroughputFactor;
					else
						out_values.aov.throughputFactor = 0.f;
					#endif
				} else
